...
- A static `import 'node:child_process'` also doesn't get the `execSync`/`execFileSync`/`spawnSync` default-timeout protection described below — that's only reachable through `dynamicImport`, and a real static import resolves through Node's own loader instead. Use `await dynamicImport('node:child_process')` in `jsk mjs` if you need the default timeout.
